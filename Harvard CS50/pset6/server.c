...
char* indexes(const char* path)
{
    ...
    // Use access F_OK to check if the file exists within path
    // if so return the path with index
    if(access("index.html", F_OK) != -1 ) 
    {
    newpath = malloc(strlen(path) + strlen(html) + 1);
    
    strcpy(newpath, path);
    strcat(newpath, html);
    newpath[strlen(path) + strlen(html)] = '\0';
    ...
    {
    newpath = malloc(strlen(path) + strlen(php) + 1);
    
    strcpy(newpath, path);
    strcat(newpath, php);
    newpath[strlen(path) + strlen(php)] = '\0';
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    char* content_of_file = malloc(sizeof(char));
    ...
    {
        content_of_file[i] = (char) c;
        i++;
        content_of_file = (char*) realloc(content_of_file, sizeof(char)*(i+1));
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char* q_point = strchr(target, '?');
    
    if(q_point == NULL)
    {
        //No query, store path, and store query as NULL
        strcpy(abs_path, target);
        query[0] = '\0';
    } else {
        //separate path and query and store both
        char path[q_point - target + 1];
        strncpy(path, target, q_point - target);
        path[q_point -target + 1] = '\0';
        strcpy(abs_path, path);
        
        strcpy(query, q_point + 1);
    }
    
    if(strstr(HTTP, "HTTP/1.1") == NULL)
    {
        error(505);
        return false;
    }
