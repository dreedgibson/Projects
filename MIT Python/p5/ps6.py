...
class Message(object):
    ...
    def apply_shift(self, shift):
        ...
        new_text = self.get_message_text()
        new_text = list(new_text)
        count = 0
        while count < len(new_text):
            try:
                new_text[count] = shift_dict[new_text[count]]
                count += 1 
            except KeyError:
                count += 1
        s = ''
        return s.join(new_text)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(26):
            wordCount = 0
            copy = Message.apply_shift(self, s)
            copy = copy.split(' ')
            for word in copy:
